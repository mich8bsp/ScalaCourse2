...
trait Solver extends GameDef:
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] = {
    val frontier: LazyList[(Block, List[Move])] = for {
      (currBlock, currHistory) <- initial
      neighbors = neighborsWithHistory(currBlock, currHistory)
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from(
    initial = neighborsWithHistory(startBlock, List[Move]()),
    explored = Set[Block](startBlock)
  )
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart
    .filter(path => done(path._1))
   ...
   * position.
   */
  lazy val solution: List[Move] = pathsToGoal.headOption.map(_._2.reverse).getOrElse(List[Move]())
